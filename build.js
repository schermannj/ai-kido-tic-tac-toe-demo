...
module.exports = __webpack_require__(10)(_, _);
...
const make_thought = name => score => cell => cell ? { name, score, thought: cell } : null;
...
const { pipe, head } = __webpack_require__(0);

const part_properties = ({ index, arr }) => ({
  next: {
    enumerable: false,
    value: () => {
      if (index < arr.length - 1) return arr[index + 1];else return undefined;
    }
  },
  has_next: {
    enumerable: false,
...
const pattern_properties = ({ space, weight = 1, name }) => ({
  ...
  fill: {
    value: function (position, available_positions, borders, pattern_part) {
      ...
      if (!cell || isBeyondWithXOffset || isBeyondWithYOffset) this.invalidate();else part.set_meta(cell.meta);

      return part.has_next() && !this.invalid ? this.fill(part.pos, available_positions, borders, part.next()) : this;
    }
  },
  invalidate: {
    value: function () {
 ...
 * @param {Array=} arr
 */
const build_part = (part, index, arr) => pipe(part_properties, props => Object.defineProperties(part, props))({ index, arr });

const build_pattern = (...parts) => ({ weight = 1.0, space, name }) => pipe(() => parts.map(build_part), pattern => Object.defineProperties(pattern, pattern_properties({ weight, space, name })))();
...
const { pipe } = __webpack_require__(0);
...
var randomFromSeed = __webpack_require__(37);
...
const { pipe, values, sortBy, filter, head, get } = __webpack_require__(0);
const { random, pattern, center } = __webpack_require__(13);
const { track } = __webpack_require__(4);
...
module.exports = {
  ...
  pattern: __webpack_require__(15),
  center: __webpack_require__(20)
};
...
const { pipe } = __webpack_require__(0);
const { get_free_cells, make_thought } = __webpack_require__(1);
const { rnd_element } = __webpack_require__(4);
 ...
 */
module.exports = (name = 'random') => (factor = 0.5) => pipe(get_free_cells, rnd_element, make_thought(name)(factor));

/***/ }),
/* 15 */
/***/ (function(module, exports, __webpack_require__) {

const { pipe, map, filter, head, flatten, sortBy } = __webpack_require__(0);
const { get_free_cells, make_thought } = __webpack_require__(1);
const patterns = __webpack_require__(16);

// use each position to build patterns
const map_available_positions = ({ board, space, available_positions }) => position => patterns({ space }).map(it => it.fill(position, available_positions, get_borders(board)));

const map_move = space => pattern => pattern ? {
  cell: pattern.find(it => it.meta.content === space).pos,
  score: pattern.score(),
  pattern
} : undefined;

const map_thought = name => factor => move => move ? Object.assign({}, make_thought(name)(factor * move.score)(move.cell), { pattern: move.pattern.meta.name }) : undefined;
...
const patterns_3x3 = __webpack_require__(17);
const patterns_4x = __webpack_require__(18);
const patterns_5x = __webpack_require__(19);

module.exports = cfg => [...patterns_3x3(cfg), ...patterns_4x(cfg), ...patterns_5x(cfg)];

/***/ }),
/* 17 */
/***/ (function(module, exports, __webpack_require__) {

const { build_pattern } = __webpack_require__(2);

/**
 * @description - the order of cells in pattern matters, basically it's the priority
 * off [x, y] - offset
 */
module.exports = ({ space }) => [build_pattern({}, { off: [1, 0] }, { off: [1, 0] })({ space, name: 'h-line' }), // x x x
build_pattern({}, // x
{ off: [0, 1] }, // x
{ off: [0, 1] // x
})({ space, name: 'v-line' }), build_pattern({}, // x
{ off: [1, 1] }, //   x
{ off: [1, 1] //     x
})({ space, name: 'd-line-right' }), build_pattern({}, //     x
{ off: [-1, 1] }, //   x
{ off: [-1, 1] // x
})({ space, name: 'd-line-left' }), build_pattern({}, // x
{ off: [0, 1] }, // x x
{ off: [1, 0] }, //      x
{ off: [1, 1] })({ space, name: 'd-line+half-v-line' }), build_pattern({}, // x
{ off: [0, 1] }, // x x x
{ off: [1, 0] }, { off: [1, 0] })({ space, name: 'h-line+half-v-line' }), build_pattern({}, //     x
{ off: [0, 1] }, // x x x
{ off: [-1, 0] }, { off: [-1, 0] })({ space, name: 'h-line+half-v-line' }), build_pattern({}, // x
{ off: [0, 1] }, // x x
{ off: [1, 0] }, // x
{ off: [-1, 1] })({ space, name: 'v-line+half-h-line' }), build_pattern({}, //   x
{ off: [0, 1] }, // x x
{ off: [-1, 0] }, //   x
{ off: [1, 1] })({ space, name: 'v-line+half-h-line-referce' })];
 ...
 * off [x, y] - offset
 */
module.exports = ({ space }) => [build_pattern({}, { off: [1, 0] }, { off: [1, 0] }, { off: [1, 0] })({ space, name: '4h-line' }), // x x x x
build_pattern({}, // x
{ off: [0, 1] }, // x
{ off: [0, 1] }, // x
{ off: [0, 1] // x
})({ space, name: '4v-line' }), build_pattern({}, // x
{ off: [1, 1] }, //   x
{ off: [1, 1] }, //     x
{ off: [1, 1] //       x
})({ space, name: '4d-line-right' }), build_pattern({}, //       x
{ off: [-1, 1] }, //     x
{ off: [-1, 1] }, //   x
{ off: [-1, 1] // x
 ...
 * off [x, y] - offset
 */
module.exports = ({ space }) => [build_pattern({}, { off: [1, 0] }, { off: [1, 0] }, { off: [1, 0] }, { off: [1, 0] })({ space, name: '5h-line' }), // x x x x x
build_pattern({}, // x
{ off: [0, 1] }, // x
{ off: [0, 1] }, // x
{ off: [0, 1] }, // x
{ off: [0, 1] // x
})({ space, name: '5v-line' }), build_pattern({}, // x
{ off: [1, 1] }, //   x
{ off: [1, 1] }, //     x
{ off: [1, 1] }, //       x
{ off: [1, 1] //         x
})({ space, name: '5d-line-right' }), build_pattern({}, //        x
{ off: [-1, 1] }, //      x
{ off: [-1, 1] }, //    x
{ off: [-1, 1] }, //  x
{ off: [-1, 1] //x
...
const { make_thought } = __webpack_require__(1);
 ...
 * payload.space: null - element that uses to mark empty cells
 */
module.exports = (name = 'center_cell') => (factor = 0.5) => payload => is_3x3_board(payload) && is_center_cell_empty(payload) ? make_thought(name)(factor)([1, 1]) : undefined;
...
"use strict";
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_shortid__ = __webpack_require__(35);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__ai_kido__ = __webpack_require__(6);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__ai_kido___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_1__ai_kido__);
...
/* harmony default export */ __webpack_exports__["a"] = ({
  ...
  data: function data() {
    var cursor = { player: 'X', ai: 'O' };

    return {
      first_move: 'player',
      boardKey: __WEBPACK_IMPORTED_MODULE_0_shortid___default()(),
      cursor: cursor,
      ai: Object(__WEBPACK_IMPORTED_MODULE_1__ai_kido__["ttt"])({ player_cursor: cursor.player, ai_cursor: cursor.ai }),
      rows: 3,
      cols: 3,
      board: []
    };
  ...
  methods: {
    start: function start() {
      ...
      this.board = new Array(parseInt(this.rows, 10)).fill(null).map(function (it) {
        return new Array(parseInt(_this.cols, 10)).fill(null);
      });

      if (this.first_move === 'ai') setTimeout(function () {
        return _this.aiMove();
      }, 300);
    },
    updateAi: function updateAi() {
      this.ai = Object(__WEBPACK_IMPORTED_MODULE_1__ai_kido__["ttt"])({ player_cursor: this.cursor.player, ai_cursor: this.cursor.ai });
    },
    aiMove: function aiMove() {
      var _ai = this.ai(this.board),
          _ai2 = _slicedToArray(_ai, 2),
          x = _ai2[0],
          y = _ai2[1];

      this.board[y][x] = this.cursor.ai;

      this.boardKey = __WEBPACK_IMPORTED_MODULE_0_shortid___default()();
    },
    cellClick: function cellClick(ri, ci) {
      var _this2 = this;

      if (this.board[ri][ci]) return;

      this.board[ri][ci] = this.cursor.player;
...
var randomByte = __webpack_require__(38);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__node_modules_vue_loader_lib_template_compiler_index_id_data_v_33f957d0_hasScoped_false_buble_transforms_node_modules_vue_loader_lib_selector_type_template_index_0_App_vue__ = __webpack_require__(43);
...
var Component = normalizeComponent(
  ...
  __WEBPACK_IMPORTED_MODULE_1__node_modules_vue_loader_lib_template_compiler_index_id_data_v_33f957d0_hasScoped_false_buble_transforms_node_modules_vue_loader_lib_selector_type_template_index_0_App_vue__["a" /* default */],
...
var update = __webpack_require__(32)("305a6384", content, true, {});
...
// module
exports.push([module.i, "#app{text-align:center;margin-top:60px;flex:1}body{background-color:#9e9e9e}label{margin:5px}h2,label{color:#fff}h2{font-weight:700}select{width:80px}input,select{outline:none;border:none;height:20px;border-radius:2px;padding-left:15px}input{width:30px}button{margin-top:10px;margin-left:10px;border:none;outline:none;background-color:#818181;border-radius:2px;height:30px;width:80px;color:#d3d3d3}button:active{filter:blur(1px)}.config{display:flex;align-items:flex-end;flex-direction:column;margin-right:48%}.board{margin-top:50px;flex:1;justify-content:center}.row{flex-direction:row}.cell{position:relative;display:inline-block;width:100px;height:100px;background-color:#ffc107;border-radius:2px;margin:0 2px}.cell:hover{filter:blur(1px)}.cell-content{color:#fff;font-family:Comic Sans MS;font-size:70px;font-weight:700;position:absolute;top:0;right:0;bottom:0;left:0}", ""]);
...
"use strict";
var render = function () {var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;return _c('div',{attrs:{"id":"app"}},[_c('h2',[_vm._v("Tic-Tac-Toe")]),_vm._v(" "),_c('div',{staticClass:"config"},[_c('label',[_vm._v("\n      Rows:\n      "),_c('input',{directives:[{name:"model",rawName:"v-model",value:(_vm.rows),expression:"rows"}],attrs:{"type":"number"},domProps:{"value":(_vm.rows)},on:{"input":function($event){if($event.target.composing){ return; }_vm.rows=$event.target.value}}})]),_vm._v(" "),_c('label',[_vm._v("\n      Cols:\n      "),_c('input',{directives:[{name:"model",rawName:"v-model",value:(_vm.cols),expression:"cols"}],attrs:{"type":"number"},domProps:{"value":(_vm.cols)},on:{"input":function($event){if($event.target.composing){ return; }_vm.cols=$event.target.value}}})]),_vm._v(" "),_c('label',[_vm._v("\n      Player cursor:\n      "),_c('input',{directives:[{name:"model",rawName:"v-model",value:(_vm.cursor.player),expression:"cursor.player"}],attrs:{"placeholder":"Player"},domProps:{"value":(_vm.cursor.player)},on:{"change":_vm.updateAi,"input":function($event){if($event.target.composing){ return; }_vm.$set(_vm.cursor, "player", $event.target.value)}}})]),_vm._v(" "),_c('label',[_vm._v("\n      AI Cursor:\n      "),_c('input',{directives:[{name:"model",rawName:"v-model",value:(_vm.cursor.ai),expression:"cursor.ai"}],attrs:{"placeholder":"AI"},domProps:{"value":(_vm.cursor.ai)},on:{"change":_vm.updateAi,"input":function($event){if($event.target.composing){ return; }_vm.$set(_vm.cursor, "ai", $event.target.value)}}})]),_vm._v(" "),_c('label',[_vm._v("\n      Player:\n      "),_c('select',{directives:[{name:"model",rawName:"v-model",value:(_vm.first_move),expression:"first_move"}],on:{"change":function($event){var $$selectedVal = Array.prototype.filter.call($event.target.options,function(o){return o.selected}).map(function(o){var val = "_value" in o ? o._value : o.value;return val}); _vm.first_move=$event.target.multiple ? $$selectedVal : $$selectedVal[0]}}},[_c('option',{attrs:{"value":"player"}},[_vm._v("Player")]),_vm._v(" "),_c('option',{attrs:{"value":"ai","selected":""}},[_vm._v("AI")])])]),_vm._v(" "),_c('button',{on:{"click":_vm.start}},[_vm._v("Render")])]),_vm._v(" "),_c('div',{key:_vm.boardKey,staticClass:"board"},_vm._l((_vm.board),function(row,ri){return _c('div',{staticClass:"row"},_vm._l((row),function(cell,ci){return _c('div',{staticClass:"cell",attrs:{"track-by":"cell"},on:{"click":function($event){_vm.cellClick(ri, ci)}}},[_c('span',{staticClass:"cell-content"},[_vm._v(_vm._s(_vm.renderCell(cell)))])])}))}))])}
var staticRenderFns = []
